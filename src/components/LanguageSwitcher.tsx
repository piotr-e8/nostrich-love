...
export function LanguageSwitcher({ className }: LanguageSwitcherProps) {
  ...
  useEffect(() => {
    ...
    if (path.startsWith("/pl/")) {
      ...
      if (savedLang && ['en', 'pl', 'es', 'de', 'zh', 'ar'].includes(savedLang)) {
  ...
  const switchLanguage = (langCode: string) => {
    ...
    if (isGuidesPage) {
      ...
      const pathWithoutLocale = currentPath.replace(/^\/(en|pl|es|de|zh|ar)(\/|$)/, '/') || '/';
      ...
      newPath = currentPath.replace(/^\/(pl|es|de|zh|ar)\//, '/');
    }
