...
1. **Seal** (kind 13) — an inner layer that encrypts the message and hides which
   key signed it
2. **Gift wrap** (kind 1059) — an outer envelope signed by a throwaway key, so
   your real identity never appears on the event that relays store

This layering means:

- Message content remains private
- **Relays cannot see who sent a message** — the gift wrap is signed by a random
  one-time key, not by you
...
<DiagramLayers
  layers={[
    { label: "Gift wrap (outer layer)", note: "Random pubkey, recipient hint — this is all a relay sees" },
    { label: "Seal (inner layer)", note: "Encrypted with an ephemeral key" },
...
Seal and gift wrap describe how a message is packaged. A third piece — the
**kind 10050 event** — describes where it is delivered. It is a small, public
list of the relays that accept your private messages.
...
Some clients publish this list automatically when you enable NIP-17. Don't
assume: verify delivery in Step 5 before relying on it.
...
relay that actually accepts gift wraps — some relays reject kind 1059 outright.
(This is [Step 3 of the migration guide](#step-3-publish-your-dm-relay-list-kind-10050) —
...
3. ✅ Publish your DM relay list (kind 10050) — without it, nothing arrives
4. ✅ Verify with contacts that they support NIP-17
5. ✅ Start using NIP-17 for all new private conversations
...
- [Your keys, your identity](/guides/keys-and-security) — losing the key loses the conversation, permanently
