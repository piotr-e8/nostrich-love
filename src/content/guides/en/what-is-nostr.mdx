...
Five pieces make up the whole protocol. You do not need them to use Nostr, but they are what people mean when they say "Nostr is simple".
...
- **Web of trust** — with no company algorithm, discovery runs on who your follows follow. That is why [finding your first people](/guides/finding-community) matters more here than on other networks.

Payments are part of the culture rather than an add-on: a **zap** is a Bitcoin Lightning tip attached to a post, denominated in sats. See [Zaps & Lightning](/guides/zaps-and-lightning) for how that works.
...
For the protocol-level version — how Nostr, ActivityPub and the AT Protocol actually differ underneath — read [Nostr vs ActivityPub vs Bluesky](/guides/protocol-comparison).
