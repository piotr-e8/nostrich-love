#!/usr/bin/env node
/**
 * Search follows of pitiunited for photography and foodie accounts
 */
...
const CATEGORY_KEYWORDS = {
  photography: [
    'photo', 'photography', 'photographer', 'camera', 'lens', 'shoot',
    'portrait', 'landscape', 'street photography', 'nature photo',
    'Nikon', 'Canon', 'Sony', 'Fujifilm', 'Leica', 'photography',
    'shutter', 'snapshot', 'pics', 'pictures', 'images', 'visual'
  ],
  foodies: [
    'chef', 'cook', 'cooking', 'food', 'recipe', 'kitchen',
    'restaurant', 'culinary', 'gastronomy', 'baking', 'cuisine',
    'foodie', 'home cook', 'food blogger', 'chef life', 'eat',
    'dining', 'meal', 'delicious', 'tasty', 'gourmet'
  ]
};

async function searchFollows() {
  console.log('🔍 Searching pitiunited follows for photography & foodie accounts...\n');
  ...
  try {
    ...
    for (const [category, categoryAccounts] of Object.entries(categorized)) {
      // Sort by score (descending)
      categoryAccounts.sort((a, b) => b.score - a.score);
      
      console.log(`\n🏷️  ${category.toUpperCase()} (${categoryAccounts.length} matches):\n`);
      
      results[category] = categoryAccounts.slice(0, 25).map((account, idx) => {
      ...
      if (categoryAccounts.length > 25) {
        console.log(`   ... and ${categoryAccounts.length - 25} more\n`);
      }
    }
    
    // Save results to file
    const outputPath = path.join(process.cwd(), 'scripts', 'pitiunited-results.json');
...
function generateTypeScriptOutput(results) {
  ...
  const tsOutputPath = path.join(process.cwd(), 'scripts', 'pitiunited-accounts.ts');
