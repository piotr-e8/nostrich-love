...
// Get new accounts
const accounts = data.newAccounts || [];
...
console.log(`Found ${accounts.length} accounts, ${uniqueAccounts.length} unique`);
...
uniqueAccounts.forEach((acc) => {
  ...
  if (acc.picture) {
    accountsOutput += `    picture: "${acc.picture}",\n`;
  }
...
console.log(`✅ Added ${uniqueAccounts.length} foodie accounts to accounts.ts`);
